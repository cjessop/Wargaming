...
%importing data from file, if not passed
     ...
     if isequal(size(alldat.dat1),size(cell(0,1)))
          dat1=cell(0,1);   %is dat1 empty
          emptyDat1 = 1;
     else%if strcmp(alldat.dat1{1,1}.SpentStageName, 'RV1')
         dat1=alldat.dat1{1,1};     %RV1
...
%Mach number
Mach=dat.mach;
if emptyDat1 == 0    %if dat1 not empty
    if size(dat1)==1  %if only one entry, dont {} index
        if ~strcmp(dat1.SpentStageName, 'RV1')
        dat1=cell(0,1);   %if dat1s first entry isnt an rv RVdat will be the same as if it were empty
        end
    elseif ~strcmp(dat1{1,1}.SpentStageName, 'RV1')
        dat1=cell(0,1);   %if dat1s first entry isnt an rv RVdat will be the same as if it were empty
    end
end


if ~isequal(size(dat1),size(cell(0,1)))
    RVdat=dat1{1,1}; 
...
if strcmp(folder, '')
    writetable(DataTable,fileName,'WriteVariableNames',0);    %save as .csv file in current wdir
else
